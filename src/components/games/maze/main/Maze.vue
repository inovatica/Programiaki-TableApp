<template>
  <div class="mainContainerMiddle" data-place="puzzle">
    <router-link :to="{name:'game-maze'}" class="backBtn"></router-link>
    <router-link :to="{name:'maze-main', params: {id: level.next }}" class="nextBtn"  v-on:click.native="changeLevel"></router-link>
    <header>
      <p class="title">
    ...
    <div class="puzzleContainer clear-fix">
      <div id="gameContent">
        ...
        <div class="bcg-flower finish-item"
             :style="'left: ' + end.col * width + 'px; top: ' + end.row * width + 'px'"></div>
      </div>
    </div>
...
<script>
  export default {
    ...
    changeLevel(){
      ...
      fillMatrix(){
        delete require.cache[require.resolve('@/components/games/maze/main/map.json')];
        let gamesMap = require('@/components/games/maze/main/map.json');
      ...
      repeatButton(){
        ...
        setTimeout(()=>{
          document.getElementsByClassName("bcg-flower")[0].classList.remove('bigger');
        }, 1000);
      ...
      getCards() {
        let _data = require('@/mok/maze_cards.json');
      ...
      win(){
        if (this.gameInfo.state !== 4) {
          document.getElementsByClassName("bcg-flower")[0].classList.add('bigger');
          this.gameInfo.state = 4;
          this.stepFlag = false;
