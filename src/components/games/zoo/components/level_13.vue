  ...
  export default {
    ...
    methods: {
      extraBehaviour(_this){
        ...
        let x = require("@/mok/cards_levels/zoo13.json");
        for(let n in x){
        ...
        if(_this.myCards.random.length === 0){
          ...
          let mult2;
          let mult3;
          for (let i in _this.cardsActual){
            // console.log(_this.cardsActual[i].object.title);
            if(_this.cardsActual[i].object.title === "*2"){
              mult2 = _this.cardsActual[i];
              mult2.show = false;
            }
            if(_this.cardsActual[i].object.title === "*3"){
              mult3 = _this.cardsActual[i];
              mult3.show = false;
            }
          }
          if(mult2 === undefined || mult3 === undefined){
            console.log("Didn't found multiple card in JSON");
          }

          let order = [];
          order.push(_this.myCards.random[0]);
          order.push(mult2);
          order.push(_this.myCards.random[1]);
          order.push(_this.myCards.random[2]);
          order.push(mult3);
          order.push(_this.myCards.random[4]);
          if (order[1].object.title === "*2" && order[2].object.title === "*2") {
            _this.myCards.random[1] = mult2;
            _this.myCards.random[4] = mult3;
          }
          else{
            _this.myCards.random = order;
          }
        }
      },
      soundButton(_this){
        ...
        const th4 = _this.myCards.random[5].object.sound;
        console.log(th1, th2, th3, th4)
        console.log(_this.myCards.random)
        ...
        arrayOfSounds.push(new Audio(require('@/components/games/zoo/sounds/' + th4)));
        arrayOfSounds.push(new Audio(require('@/components/games/zoo/sounds/' + th4)));
        arrayOfSounds.push(new Audio(require('@/components/games/zoo/sounds/' + th4)));
