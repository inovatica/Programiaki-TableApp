    ...
    methods: {
      ...
      findMeCard(name, _this) {
        ...
        for (let card of _this.cardsActual) {
          if (card.object.title === name) {
            console.log(_this.cardsActual.length)
            return card;
          }
        }
        console.log(_this.cardsActual.length)
      ...
      randomCards(_this) {
        ...
        if(_this.gameInfo.levels[14].data !== undefined){
          let previousLvlData;
          for (let i = 0; i < _this.gameInfo.levels[13].cardsCount; i++) {
            previousLvlData = _this.gameInfo.levels[13].data[i];
        ...
        for (let i = 0; i < (_this.gameInfo.levels[14].cardsCount) / 2; i++) {
          ...
          if (_this.gameInfo.levels[13].data !== undefined && i === 0){
            _this.myCards.random.length = 0;
            _this.myCards.random = _this.gameInfo.levels[13].data;
          }
          else{
            _this.cardsActual.splice(rand,1);
          }
        }
        _this.gameInfo.levels[14].data = JSON.parse(JSON.stringify(_this.myCards.random));
      },
      ...
      validate(_this){
        ...
        if(countOnTable === 2 && _this.myCards.random[1].show === true && _this.myCards.random[3].show === true){
          _this.win();
        }
        else{
          _this.wrongWay();
